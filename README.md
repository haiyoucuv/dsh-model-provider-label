...
Harness composer model seat, so identical model names from different providers
become distinguishable.

默认模型选择器只显示模型名（如 `DeepSeek V4 Flash`）。当你配置多个 provider、
且不同 provider 下有**同名的模型**时无法区分。这个插件把 provider 的
displayName 也显示出来：**`DeepSeek · DeepSeek V4 Flash`**。
...
本插件只是**遮蔽**了 `conversation.input.model` UI 槽位，用 priority 更低的
组件替换默认渲染，其余逻辑 100% 复用宿主 `modelDirectories` 服务。
...
# 方式二：从 git 仓库安装（分享后）
dsh plugin --profile web add https://github.com/<you>/dsh-model-provider-label/archive/refs/tags/v0.1.0.tar.gz
```
...
> ```sh
> cd ~/.dsh/profiles/web && pnpm add link:/path/to/dsh-model-provider-label
> ```
...
与 /model 命令同一份状态）。翻译直接复用宿主已注册的 `model` locale namespace（不重复注册）。
